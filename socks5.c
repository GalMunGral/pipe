...
int handshake(int src_sock)
{
  ...
  case 0x03:
  {
    ...
    if (getaddrinfo(DST_ADDR, PORT, &hints, &res) != 0)
    {
    ...
    {
      if ((dst_sock = socket(cur->ai_family, cur->ai_socktype, cur->ai_protocol)) < 0)
        continue;
      if (connect(dst_sock, cur->ai_addr, cur->ai_addrlen) == 0)
      {
        printf("SUCCESS\n");
        break;
      }
  ...
  case 0x04:
  {
    ...
    if (send(src_sock, &addr.sin6_addr, 4, 0) < 0)
    {
      fprintf(stderr, "handshake << BND_ADDR\n");
      close(dst_sock);
      return -1;
...
void loop(int sock1, int sock2)
{
  ...
  for (;;)
  {
    int res = poll((pollfd_t *)&fds, 2, 1000);
